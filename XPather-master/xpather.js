...
function toggleSidebar() {
	if ($xpather.is(':visible')) {
		...
		button.setAttribute('z-index', '1999');
		button.style.width = '100px';
		button.style.height = '50px';
		button.style.background = 'black';
		button.style.color = 'white';
		//button.setAttribute('ondblclick', 'document.body.removeChild(button)');
		...
		button.addEventListener('click', function() {
			...
			let arr = [];
			const listResults = document.getElementsByClassName("xpather-sidebar-entry "+copyClassName);
			for(let index = 0; index < listResults.length; index++) {
				const value = listResults[index].getAttribute('value');
				arr.push(value);
			}
			catalog = arr.join("\n\n");
		
			var x = document.createElement("TEXTAREA");
  			var t = document.createTextNode(catalog);
  			x.appendChild(t);
